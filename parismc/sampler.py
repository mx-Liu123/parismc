...
class Sampler:
    ...
    def initialize_first_iteration(self, num_iterations: int,
        ...
        for i in range(self.n_seed):
            ...
            self.inv_covariances_list.append(np.empty((int(self.maximum_array_size / self.cov_update_count) + 1, self.ndim, self.ndim)))
            self.gaussian_normterm_list.append(np.empty((int(self.maximum_array_size / self.cov_update_count) + 1,)))
